...
class GVFScaredGhostList(GVFList):

    def __init__(self, params, game_state):

        GVFList.__init__(self, params, game_state)

        self.pseudo_reward = self.params['gvf_scared_ghost_pseudo_reward']
    ...
    def learn(self, current_game_state, action):
        ...
        for scared_ghost_gvf_location in self.gvf_map.keys():

            if scared_ghost_gvf_location != next_pacman_location:

                self.gvf_map[scared_ghost_gvf_location].learn(pacman_location, action, self.pseudo_negative_reward,
                                                       next_pacman_location, False)

            else:
                self.gvf_map[scared_ghost_gvf_location].learn(pacman_location, action, self.pseudo_reward,
                                                       next_pacman_location, True)
